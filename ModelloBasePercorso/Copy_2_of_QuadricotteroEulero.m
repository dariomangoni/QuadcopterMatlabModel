...
vel_laterale_max=3;
distanza_raggiungimento_vel=1.5;
vel_salita_max=1.5;
vel_discesa_max=-1.5;
...
for stp=2:stp_fin
    ...
    if ( (err_x(stp)^2 + err_x(stp)^2 + err_x(stp)^2)<toll_waypoint )
...
%         v_mot(mot)= earth_to_body( [0;0;1] , quat(stp-1,:)' )' * vel(stp-1,:)';
        ...
        if ctrl_rpm(stp,mot) > 10000
            ctrl_rpm(stp,mot) = 10000;
        elseif ctrl_rpm(stp,mot)<-10000
            ctrl_rpm(stp,mot) = -10000;
        end
