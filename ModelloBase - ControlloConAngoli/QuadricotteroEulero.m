...
% Aa=kpa+kia*dt_ctrl/2+kda/dt_ctrl;
% Ba=kia*dt_ctrl/2-kda/dt_ctrl;
% Ca=kia;
...
for stp=2:stp_fin
    ...
    %     err_y_m(stp)=real(sqrt(err_x(stp)^2+err_y(stp)^2-err_x_m(stp)^2));
        ...
        %% Inclinazione
        if err_y_m(stp)>distanza_raggiungimento_incl_max
            inclinazione_y_set(stp) = incl_max;
        elseif err_y_m(stp)<-distanza_raggiungimento_incl_max
            inclinazione_y_set(stp) = -incl_max;
        else
            inclinazione_y_set(stp)=err_y_m(stp)*incl_max/distanza_raggiungimento_incl_max;
        end
        
%         inclinazione_y_set(stp) = 10;
        ...
        if err_x_m(stp)>distanza_raggiungimento_incl_max
            inclinazione_x_set(stp) = incl_max;
        elseif err_x_m(stp)<-distanza_raggiungimento_incl_max
            inclinazione_x_set(stp) = -incl_max;
        else
            inclinazione_x_set(stp)=err_x_m(stp)*incl_max/distanza_raggiungimento_incl_max;
        end

%         inclinazione_x_set(stp) = 0;
...
plot((1:stp_fin)*dt,asind(inclinazione_x_set(1:stp_fin)/100),'r');
plot((1:stp_fin)*dt,asind(inclinazione_y_set(1:stp_fin)/100),'c');
